...
The longer-term direction is a live set visualizer where each setlist song has
its own video, but do not build that runtime until the prototype needs it.
...
- Do not add routing, pages, song registries, visualizer registries, content APIs, or multi-song config until live setlist behavior is the active task.
- Keyboard or MIDI-oriented action hooks are acceptable when they stay compact and do not force a setlist architecture.
...
- If visual behavior changes, sanity-check that fullscreen, keyboard controls, live navigation placeholders, video playback, and shader uniforms still work.
