...
run_render_tier() {
  ...
  if [[ ! -d "$fixture_dir/remotion-src/node_modules" ]]; then
    echo "    ⏳ pnpm install (first run, lifecycle scripts disabled)"
    (cd "$fixture_dir/remotion-src" && "$PNPM_BIN" install --frozen-lockfile --ignore-scripts --silent >/dev/null 2>&1)
  fi

  echo "    ⏳ render Remotion baseline"
  if ! (cd "$fixture_dir/remotion-src" && \
        "$PNPM_BIN" exec remotion render "$composition_id" out/baseline.mp4 >/dev/null 2>&1); then
